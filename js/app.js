...
import { yoNuncaNuncaData } from '../data/yonuncanunca/data.js';
...
import { createNavigation } from './navigation.js?v=20260917yonuncanunca';
...
} from './config.js?v=20260917yonuncanunca';
...
(() => {
  ...
  function exitLocalGameToHome(){
    clearCountdown();clearMiniCountdown();clearChupisticaTimers();
    state.whoamiLocal=null;state.ageLocal=null;state.confessionsLocal=null;state.chamuyayaLocal=null;
    state.chupistica=null;state.whatWouldYouDoConfig=null;state.yoNuncaNunca=null;
    state.lastRoomData=null;state.roomCode='';state.mode=null;state.gameType=null;resetHistory();show('home',{history:false});
  }
  function handleBrandHome(){
    if(['home','access'].includes(state.currentScreen))return;
    if(['authLogin','authRegister'].includes(state.currentScreen)){resetHistory();show('access',{history:false});return;}
    if(state.roomRef){void requestLeaveRoom();return;}
    exitLocalGameToHome();
  }
  ...
  async function enterGuest(){
    if(state.guestSignInInFlight)return false;
    ...
    // El acceso como invitado siempre comienza en HOME. Nunca debe heredar
    // el contexto en memoria de un generador local anterior.
    state.yoNuncaNunca=null;
    if(!state.roomRef){state.mode=null;state.gameType=null;state.lastRoomData=null;}
    setButtonBusy('guestBtn','account',true,'ENTRANDO…');state.guestSignInInFlight=true;
    try{
      await auth.setPersistence(firebase.auth.Auth.Persistence.SESSION);
      const existing=auth.currentUser;const session=readSession();
      if(existing&&!existing.isAnonymous){state.guestSignInInFlight=false;state.guestMode=false;state.authUser=existing;renderAccountUI();resetHistory();show('home',{history:false});return true;}
    ...
    }catch(error){accountNotice('accessNotice',accountError(error),'error');return false;}finally{state.guestSignInInFlight=false;setButtonBusy('guestBtn','account',false);}
  }
  ...
  async function createMiniRoom(type){
    ...
    const stopConfig=gameType===GAME_TYPES.STOP?stopConfigFromUI():null,confessionsConfig=gameType===GAME_TYPES.CONFESSIONS?confessionsConfigFromUI(confessionsModeFromUI()):null,chamuyayaConfig=gameType===GAME_TYPES.CHAMUYA?defaultChamuyayaConfig():null,tribunalConfig=gameType===GAME_TYPES.TRIBUNAL?tribunalConfigFromUI():null,whatWouldYouDoConfig=gameType===GAME_TYPES.WHAT_WOULD_YOU_DO?whatWouldYouDoConfigFromUI():null,totalRounds=gameType===GAME_TYPES.STOP?Number(stopConfig.totalRounds)||3:gameType===GAME_TYPES.CONFESSIONS?0:gameType===GAME_TYPES.CHAMUYA?CHAMUYA_DEFAULT_ROUNDS:gameType===GAME_TYPES.TRIBUNAL?Number(tribunalConfig.totalRounds)||TRIBUNAL_DEFAULT_ROUNDS:gameType===GAME_TYPES.WHAT_WOULD_YOU_DO?Number(whatWouldYouDoConfig.totalRounds)||3:Number(state.miniConfig?.totalRounds)||3,nameInputId=gameType===GAME_TYPES.STOP?'stopHostNameInput':gameType===GAME_TYPES.CONFESSIONS?'confessionsHostNameInput':gameType===GAME_TYPES.CHAMUYA?'chamuyayaHostNameInput':gameType===GAME_TYPES.AGE?'ageHostNameInput':gameType===GAME_TYPES.WHAT_WOULD_YOU_DO?'whatWouldYouDoHostNameInput':'tribunalHostNameInput',createButtonId=gameType===GAME_TYPES.STOP?'stopCreateRoomBtn':gameType===GAME_TYPES.CONFESSIONS?'confessionsCreateRoomBtn':gameType===GAME_TYPES.CHAMUYA?'chamuyayaCreateRoomBtn':gameType===GAME_TYPES.AGE?'ageCreateRoomBtn':gameType===GAME_TYPES.WHAT_WOULD_YOU_DO?'whatWouldYouDoCreateRoomBtn':'tribunalCreateRoomBtn',noticeId=gameType===GAME_TYPES.STOP?'stopSetupNotice':gameType===GAME_TYPES.CONFESSIONS?'confessionsSetupNotice':gameType===GAME_TYPES.CHAMUYA?'chamuyayaSetupNotice':gameType===GAME_TYPES.AGE?'ageSetupNotice':gameType===GAME_TYPES.WHAT_WOULD_YOU_DO?'whatWouldYouDoSetupNotice':'tribunalSetupNotice';
  ...
  async function startChamuyayaRound(data,players=chamuyayaRoundPlayers(data)){
    ...
    const config=data.settings?.chamuyaya||defaultChamuyayaConfig(),count=Math.max(1,Math.min(players.length-1,Number(config.chaMuyaCount)||1)),game=data.game||{},usedDatoIds=new Set(Array.isArray(game.chamuyaya?.usedDataIds)?game.chamuyaya.usedDataIds.map(Number):[]),available=chamuyayaCatalog.filter(item=>!usedDatoIds.has(Number(item.id))),totalRounds=Math.max(1,Number(game.totalRounds)||Number(config.totalRounds)||CHAMUYA_DEFAULT_ROUNDS),round=Number(game.currentRound||0)+1,remainingRounds=Math.max(1,totalRounds-round+1),requiredDataCount=remainingRounds;
    ...
    const selectedData=shuffleArray(available)[0],selectedDataId=Number(selectedData.id),token=miniToken(GAME_TYPES.CHAMUYA,round),chaIds=shuffleArray(players.map(player=>String(player.id))).slice(0,count),assignments={};
    players.forEach(player=>{const id=String(player.id),isCha=chaIds.includes(id);assignments[id]={kind:GAME_TYPES.CHAMUYA,playerId:id,round,roundToken:token,role:isCha?'chamuyaya':'normal',data:isCha?null:{id:Number(selectedData.id),dato:selectedData.dato},ready:false};});
    usedDatoIds.add(selectedDataId);
    const countdownEndsAt=serverNow()+CHAMUYA_COUNTDOWN_DURATION_MS,nextGame={...game,gameType:GAME_TYPES.CHAMUYA,phase:'chamuyayaCountdown',round,currentRound:round,totalRounds:totalRounds,countdownEndsAt,roundEndsAt:null,roundToken:token,roundResults:null,scores:miniScores(game,players),playerNames:Object.fromEntries(players.map(player=>[String(player.id),player.name])),activePlayers:Object.fromEntries(players.map(player=>[String(player.id),true])),chamuyaya:{estado:'cuenta_regresiva',countdownEndsAt,chaMuyaCount:count,usedDataIds:[...usedDatoIds],secretDataId:selectedDataId,secretRoleIds:chaIds,ready:{},votes:{},voteSubmitted:{}}};
  ...
  function yoNuncaNuncaOriginScreen(){
    return state.currentScreen==='minigames'?'minigames':'access';
  }
  function startYoNuncaNuncaGame(returnScreen=null){
    const origin=returnScreen||state.yoNuncaNunca?.returnScreen||yoNuncaNuncaOriginScreen();
    // Este flujo solo usa el mazo local en memoria: no crea sesión, sala ni
    // ninguna operación de autenticación o base de datos.
    state.yoNuncaNunca={deck:shuffleArray(yoNuncaNuncaData),index:-1,returnScreen:origin};
    state.mode='local';state.gameType=null;state.roomCode='';state.lastRoomData=null;
    resetHistory();show('yoNuncaNunca',{history:false});nextYoNuncaNuncaPhrase();
  }
  function openYoNuncaNuncaOffline(){
    startYoNuncaNuncaGame(yoNuncaNuncaOriginScreen());
  }
  function renderYoNuncaNunca(){
    const game=state.yoNuncaNunca,item=game?.deck?.[game.index];
    if(!game||!item)return;
    setText('yoNuncaNuncaPhrase',item.frase);
    setText('yoNuncaNuncaCount',`${game.index+1} / ${game.deck.length}`);
    const next=$('yoNuncaNuncaNextBtn');
    if(next)next.disabled=game.index>=game.deck.length-1;
  }
  function nextYoNuncaNuncaPhrase(){
    const game=state.yoNuncaNunca;
    if(!game||game.index>=game.deck.length-1)return;
    game.index++;renderYoNuncaNunca();
  }
  function restartYoNuncaNunca(){startYoNuncaNuncaGame(state.yoNuncaNunca?.returnScreen||'access');}
  function exitYoNuncaNunca(){
    const destination=['access','minigames'].includes(state.yoNuncaNunca?.returnScreen)
      ? state.yoNuncaNunca.returnScreen : 'access';
    state.yoNuncaNunca=null;state.lastRoomData=null;state.roomCode='';state.mode=null;state.gameType=null;
    resetHistory();show(destination,{history:false});
  }
  ...
  function startChamuyayaLocalRound(){const game=state.chamuyayaLocal||createChamuyayaLocalState(),chaCount=Math.max(1,Math.min(game.players.length-1,Number(game.chaMuyaCount)||1)),usedDatoIds=new Set(Array.isArray(game.usedDataIds)?game.usedDataIds.map(Number):[]),available=chamuyayaCatalog.filter(item=>!usedDatoIds.has(Number(item.id))),remainingRounds=Math.max(1,Number(game.totalRounds||CHAMUYA_DEFAULT_ROUNDS)-Number(game.round||1)+1),requiredDataCount=remainingRounds;if(available.length<requiredDataCount){miniNotice('chamuyayaLocalNotice','No hay suficientes datos distintos para terminar esta partida sin repeticiones. Se necesitan '+requiredDataCount+' y quedan '+available.length+'. Agrega más datos en data/chamuyaya/data.js.','error');return;}const selectedData=shuffleArray(available)[0],selectedDataId=Number(selectedData.id);usedDatoIds.add(selectedDataId);game.chaMuyaIds=shuffleArray(game.players.map(player=>player.id)).slice(0,chaCount);const normalPlayers=game.players.filter(player=>!game.chaMuyaIds.includes(player.id));game.chaMuyaCount=chaCount;game.dataIdsByPlayer=Object.fromEntries(normalPlayers.map(player=>[player.id,selectedDataId]));game.dataId=selectedDataId;game.usedDataIds=[...usedDatoIds];game.phase='reveal';game.revealIndex=0;game.revealVisible=false;game.discussionVisible=false;game.selectedVotes=[];game.voterIndex=0;game.votes=[];localChamuyayaSave();show('chamuyayaLocalReveal');renderChamuyayaLocalReveal();}
  ...
  const navigation = createNavigation({
    ...
    onScreenChange: screenId => {
      if (screenId !== 'yoNuncaNunca' && state.yoNuncaNunca) {
        state.yoNuncaNunca = null;
        if (state.mode === 'local' && !state.roomRef) state.mode = null;
      }
      rulesController?.sync(screenId);
    }
  });
  ...
  connection.start();
  $('pageBrand').onclick=handleBrandHome;
  ...
  $('yoNuncaNuncaGameBtn')?.addEventListener('click',openYoNuncaNuncaOffline);
  $('offlineYoNuncaNuncaBtn')?.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();openYoNuncaNuncaOffline();});
  ...
  $('guestBtn').onclick=event=>{event.preventDefault();event.stopPropagation();void enterGuest();};
  ...
  $('yoNuncaNuncaNextBtn')?.addEventListener('click',nextYoNuncaNuncaPhrase);$('yoNuncaNuncaRestartBtn')?.addEventListener('click',restartYoNuncaNunca);$('yoNuncaNuncaBackBtn')?.addEventListener('click',exitYoNuncaNunca);
  ...
  authController.start(async user=>{
    ...
    if(user&&!user.isAnonymous){
      ...
      if(user?.isAnonymous){
        ...
        if(!state.guestSignInInFlight&&!state.roomRef&&!guestMarker&&session?.authUid!==user.uid){try{await auth.signOut();}catch(error){console.warn('anonymous isolation',error);}state.guestMode=false;state.authUser=null;renderAccountUI();showSavedSessionBanner();return;}
    ...
    renderAccountUI();
    if(user&&!user.isAnonymous&&!state.roomRef&&!state.autoRestoring&&state.splashDone&&!state.yoNuncaNunca){resetHistory();show('home',{history:false});}
    if(user?.isAnonymous&&state.guestMode&&!state.roomRef&&state.splashDone&&!state.yoNuncaNunca){resetHistory();show('home',{history:false});}
  ...
  async function finishSplash(force=false){
    ...
    if(state.roomRef)return;
    if(state.yoNuncaNunca){show('yoNuncaNunca',{history:false});renderYoNuncaNunca();return;}
